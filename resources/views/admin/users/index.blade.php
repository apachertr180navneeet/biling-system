...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="m-page-header">
        <div class="m-page-header-left">
            ...
            <div>
                <h4 class="m-page-title">Users</h4>
        ...
        <a href="{{ route('admin.users.create') }}" class="btn btn-primary m-btn-add"><i class="bx bx-plus"></i> Add User</a>
        @endif
    </div>
...
@section('script')
<script>
function usersStatusBadge(status, row) {
    ...
    return '<button type="button" class="btn btn-sm btn-status-toggle btn-' + (status === 'active' ? 'success' : 'warning') + '" data-url="' + row.status_url + '">' + status.charAt(0).toUpperCase() + status.slice(1) + '</button>';
    @else
    return '<span class="badge bg-label-' + (status === 'active' ? 'success' : 'warning') + '">' + status.charAt(0).toUpperCase() + status.slice(1) + '</span>';
    @endif
}

function usersActions(row) {
...
var table = $('#users-table').DataTable({
    ...
    columns: [
        ...
        { data: 'status', name: 'status', orderable: false, searchable: false, render: function(data, type, row) {
            return usersStatusBadge(data, row);
        }},
        { data: 'actions', name: 'actions', orderable: false, searchable: false, render: function(data, type, row) {
            return usersActions(row);
        }}
    ],
    order: [[0, 'desc']]
});
