...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card"><div class="card-body">
        <div class="text-center mb-4">
            ...
            <h5>{{ $spareSale->is_gst ? 'TAX INVOICE / COUNTER SALE' : 'RETAIL INVOICE / CASH MEMO' }}</h5>
        </div>
        ...
        <table class="table table-bordered table-sm mt-3">
            ...
            <tfoot>
                <tr><td colspan="{{ $spareSale->is_gst ? 5 : 4 }}"></td><td><strong>Subtotal:</strong></td><td class="text-end">{{ number_format($spareSale->subtotal, 2) }}</td></tr>
                @if($spareSale->is_gst)
                <tr><td colspan="5"></td><td><strong>GST:</strong></td><td class="text-end">{{ number_format($spareSale->gst_amount, 2) }}</td></tr>
                @endif
                <tr><td colspan="{{ $spareSale->is_gst ? 5 : 4 }}"></td><td><strong>Grand Total:</strong></td><td class="text-end">{{ number_format($spareSale->grand_total, 2) }}</td></tr>
            </tfoot>
        </table>
