...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold mb-4">Create Vehicle Master</h4>
        <div class="card"><div class="card-body">
        <form method="POST" action="{{ route('admin.vehicle-masters.store') }}">
            @csrf
            <div class="row g-3">
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" step="0.01" name="ex_showroom_price" class="form-control @error('ex_showroom_price') is-invalid @enderror" value="{{ old('ex_showroom_price', '0') }}" required>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="battery_type" class="form-control @error('battery_type') is-invalid @enderror" value="{{ old('battery_type') }}" placeholder="e.g. LITHIUM">
                    @error('battery_type')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label">Make of Battery</label>
                    <input type="text" name="battery_make" class="form-control @error('battery_make') is-invalid @enderror" value="{{ old('battery_make') }}" placeholder="e.g. LITHIUM">
                    @error('battery_make')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
            </div>
            <div class="mt-4"><button type="submit" class="btn btn-primary">Save</button> <a href="{{ route('admin.vehicle-masters.index') }}" class="btn btn-secondary">Cancel</a></div>
        </form>
    </div></div>
</div>
@endsection
