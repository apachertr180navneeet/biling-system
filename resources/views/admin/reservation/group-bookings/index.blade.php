...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="m-page-header">
        <div class="m-page-header-left">
            ...
            <div>
                <h4 class="m-page-title">Group Bookings</h4>
                <ul class="m-breadcrumb">
                    <li><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                    <li class="m-breadcrumb-sep">/</li>
                    <li><a href="#">Reservations</a></li>
                    <li class="m-breadcrumb-sep">/</li>
                    <li class="active">Group Bookings</li>
                </ul>
            </div>
        </div>
        @if(auth()->user()->hasPermission('group_bookings.create'))
        <a href="{{ route('admin.reservation.group-bookings.create') }}" class="btn btn-primary m-btn-add"><i class="bx bx-plus"></i> Add Group Booking</a>
        @endif
    </div>

    <div class="card">
        <div class="card-body">
            <table id="group-bookings-table" class="table table-bordered table-striped" style="width:100%">
                <thead><tr><th>#</th><th>Reservation #</th><th>Group Name</th><th>Contact</th><th>Hotel</th><th>Rooms</th><th>Check-In</th><th>Check-Out</th><th>Amount</th><th>Status</th><th>Actions</th></tr></thead>
            </table>
        </div>
    </div>
</div>
...
function groupBookingStatusBadge(status, statusUrl) {
    var color = statusColors[status] || 'secondary';
    @if(auth()->user()->hasPermission('group_bookings.edit'))
...
function groupBookingActions(row) {
    ...
    @endif
    @if(auth()->user()->hasPermission('group_bookings.delete'))
...
var table = $('#group-bookings-table').DataTable({
    processing: true,
    serverSide: true,
    ajax: "{{ route('admin.reservation.group-bookings.data') }}",
    columns: [
        ...
        { data: 'room_count', name: 'room_count', orderable: false, searchable: false },
{ data: 'check_in_date', name: 'check_in_date', render: function(data) { return data ? moment(data).format('DD-MM-YYYY') : ''; }},
        ...
        { data: 'status', name: 'status', orderable: false, searchable: false, render: function(data, type, row) {
            return groupBookingStatusBadge(data, row.status_url);
        }},
        { data: 'actions', name: 'actions', orderable: false, searchable: false, render: function(data, type, row) {
            return groupBookingActions(row);
        }}
    ],
    order: [[0, 'desc']]
});
