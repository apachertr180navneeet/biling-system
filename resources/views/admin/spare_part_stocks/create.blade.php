...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light">Admin / Spare Part Stock /</span> Create
    </h4>
    <div class="card">
        <div class="card-header"><h5 class="mb-0">New Stock Entry</h5></div>
        <div class="card-body">
            <form method="POST" action="{{ route('admin.spare-part-stocks.store') }}">
                @csrf
                <div class="mb-3">
                    ...
                    <select name="spare_part_id" class="form-control @error('spare_part_id') is-invalid @enderror">
                        ...
                        <option value="{{ $part->id }}" {{ old('spare_part_id') == $part->id ? 'selected' : '' }}>{{ $part->part_no }} - {{ $part->name }} ({{ $part->category->name ?? '' }})</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="quantity" class="form-control @error('quantity') is-invalid @enderror" value="{{ old('quantity', 0) }}" min="0">
                    @error('quantity') <div class="text-danger small">{{ $message }}</div> @enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Minimum Quantity (Reorder Level)</label>
                    <input type="number" name="min_quantity" class="form-control @error('min_quantity') is-invalid @enderror" value="{{ old('min_quantity', 0) }}" min="0">
