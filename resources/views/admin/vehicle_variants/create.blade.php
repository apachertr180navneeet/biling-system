...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light">Admin / Vehicle Variants /</span> Create
    </h4>
    <div class="card">
        <div class="card-header"><h5 class="mb-0">New Variant</h5></div>
        <div class="card-body">
            <form method="POST" action="{{ route('admin.vehicle-variants.store') }}">
                @csrf
                <div class="mb-3">
                    ...
                    <select name="model_id" class="form-control @error('model_id') is-invalid @enderror">
                        ...
                        <option value="{{ $model->id }}" {{ old('model_id') == $model->id ? 'selected' : '' }}>{{ $model->brand->name ?? '' }} {{ $model->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="fuel_type" class="form-control @error('fuel_type') is-invalid @enderror" value="{{ old('fuel_type') }}">
                    @error('fuel_type') <div class="text-danger small">{{ $message }}</div> @enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Transmission</label>
                    <input type="text" name="transmission" class="form-control @error('transmission') is-invalid @enderror" value="{{ old('transmission') }}">
                    @error('transmission') <div class="text-danger small">{{ $message }}</div> @enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Ex-Showroom Price</label>
                    <input type="number" step="0.01" name="ex_showroom_price" class="form-control @error('ex_showroom_price') is-invalid @enderror" value="{{ old('ex_showroom_price') }}">
