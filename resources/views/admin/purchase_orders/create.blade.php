            ...
            <form method="POST" action="{{ route('admin.purchase-orders.store') }}" id="poForm">
                ...
                <div id="itemsContainer">
                    <div class="item-row row mb-2 align-items-center">
                        <div class="col-md-5">
                            <select name="items[0][spare_part_id]" class="form-select spare-part-select" required>
                                <option value="">Select Part</option>
                                @foreach($spareParts as $part)
                                <option value="{{ $part->id }}" data-price="{{ number_format($part->purchase_price, 2, '.', '') }}">{{ $part->part_no }} - {{ $part->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-2">
