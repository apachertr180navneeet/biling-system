            ...
            <form method="POST" action="{{ route('admin.purchase-orders.update', $purchaseOrder) }}" id="poForm">
                ...
                <div id="itemsContainer">
                    ...
                    <div class="item-row row mb-2 align-items-center">
                        <div class="col-md-5">
                            <select name="items[{{ $i }}][spare_part_id]" class="form-select spare-part-select" required>
                                <option value="">Select Part</option>
                                @foreach($spareParts as $part)
                                <option value="{{ $part->id }}" data-price="{{ number_format($part->purchase_price, 2, '.', '') }}" {{ $item->spare_part_id == $part->id ? 'selected' : '' }}>{{ $part->part_no }} - {{ $part->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-2">
                ...
                <button type="button" class="btn btn-sm btn-secondary mt-2" id="addItem">+ Add Row</button>
...
$(document).ready(function() {
    ...
    $('#addItem').click(function() {
        ...
        @foreach($spareParts as $part)
        optionsHtml += '<option value="{{ $part->id }}" data-price="{{ number_format($part->purchase_price, 2, '.', '') }}">{{ addslashes($part->part_no) }} - {{ addslashes($part->name) }}</option>';
        @endforeach

        var html = '<div class="item-row row mb-2 align-items-center">' +
            '<div class="col-md-5"><select name="items[' + itemIndex + '][spare_part_id]" class="form-select spare-part-select" required>' + optionsHtml + '</select></div>' +
            '<div class="col-md-2"><input type="number" name="items[' + itemIndex + '][quantity]" class="form-control qty text-center" placeholder="Qty" min="1" value="1" required></div>' +
            '<div class="col-md-2"><input type="number" step="0.01" name="items[' + itemIndex + '][unit_price]" class="form-control unit-price" placeholder="Price" min="0" value="0.00"></div>' +
            '<div class="col-md-2"><input type="text" class="form-control line-total" readonly value="0.00"></div>' +
            '<div class="col-md-1 text-center"><button type="button" class="btn btn-sm btn-danger remove-item" title="Remove">X</button></div>' +
        '</div>';
