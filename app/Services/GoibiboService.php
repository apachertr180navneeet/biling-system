...
class GoibiboService
{
    ...
    public function pullReservations(): array
    {
        ...
        try {
            $response = $this->makeRequest('GET', '/bookings', [
                'hotel_id' => $this->channel->property_id_on_ota,
            ]);

            $this->updateSyncLog($log, $response, 'success');

            return $this->processReservations($response['bookings'] ?? []);
        } catch (Exception $e) {
    ...
    public function pushAvailability(array $availability): array
    {
        ...
        try {
            $response = $this->makeRequest('PUT', '/inventory', [
                'hotel_id' => $this->channel->property_id_on_ota,
                'rooms' => $availability,
            ]);
    ...
    public function cancelReservation(string $otaReservationId): array
    {
        ...
        try {
            $response = $this->makeRequest('DELETE', "/bookings/{$otaReservationId}");
    ...
    private function makeRequest(string $method, string $endpoint, array $data = []): array
    {
        ...
        $response = Http::withHeaders([
            ...
            'X-Goibibo-Hotel-ID' => $this->channel->property_id_on_ota,
        ])->timeout(30)->send($method, $url, $data);

        if ($response->failed()) {
            throw new Exception("Goibibo API error: {$response->status()} - {$response->body()}");
        }
