...
class AirbnbService
{
    ...
    public function pullReservations(): array
    {
        ...
        try {
            $response = $this->makeRequest('GET', '/reservations', [
                'listing_id' => $this->channel->property_id_on_ota,
            ]);
    ...
    public function pushAvailability(array $availability): array
    {
        ...
        try {
            $response = $this->makeRequest('PUT', '/calendar', [
                'listing_id' => $this->channel->property_id_on_ota,
    ...
    public function pushRates(array $rates): array
    {
        ...
        try {
            $response = $this->makeRequest('PUT', '/pricing', [
                'listing_id' => $this->channel->property_id_on_ota,
                'pricing' => $rates,
            ]);
    ...
    public function cancelReservation(string $otaReservationId): array
    {
        ...
        try {
            $response = $this->makeRequest('POST', "/reservations/{$otaReservationId}/cancel");
    ...
    private function makeRequest(string $method, string $endpoint, array $data = []): array
    {
        ...
        $response = Http::withHeaders([
            ...
            'X-Airbnb-Listing-ID' => $this->channel->property_id_on_ota,
        ])->timeout(30)->send($method, $url, $data);

        if ($response->failed()) {
            throw new Exception("Airbnb API error: {$response->status()} - {$response->body()}");
        }
