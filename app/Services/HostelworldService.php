...
class HostelworldService
{
    ...
    public function pullReservations(): array
    {
        ...
        try {
            $response = $this->makeRequest('GET', '/reservations', [
                'property_id' => $this->channel->property_id_on_ota,
            ]);
    ...
    private function makeRequest(string $method, string $endpoint, array $data = []): array
    {
        ...
        $response = Http::withHeaders([
            ...
            'X-Hostelworld-Property-ID' => $this->channel->property_id_on_ota,
        ])->timeout(30)->send($method, $url, $data);

        if ($response->failed()) {
            throw new Exception("Hostelworld API error: {$response->status()} - {$response->body()}");
        }
